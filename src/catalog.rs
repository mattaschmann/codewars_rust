...
fn catalog(s: &str, article: &str) -> String {
    let re = Regex::new(r".*<name>(.*)</name><prx>(.*)</prx><qty>(.*)</qty>").unwrap();

    let res = re.captures_iter(s)
        .filter_map(|cap| match &cap[1].contains(article) {
            true => Some(format!("{} > prx: ${} qty: {}", &cap[1], &cap[2], &cap[3])),
            false => None,
        })
        .collect::<Vec<String>>();

    if res.is_empty() { return String::from("Nothing"); }
...
mod tests {
    ...
    fn basic_tests() {
        ...
        dotest(s, "saw", "table saw > prx: $1099.99 qty: 5\nsaw > prx: $9 qty: 10\nsaw for metal > prx: $13.80 qty: 32");
    }
}
